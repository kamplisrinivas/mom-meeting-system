...
export default function Dashboard() {
  ...
  const openMomForm = (meeting) => {
    // Navigates to the All Meetings page instead of a specific MOM point form
    navigate(`/meetings`);
  };
...
const MeetingCard = ({ meeting, onClick, token }) => (
  <div style={styles.meetingCard} onClick={onClick}>
    ...
    <div style={styles.meetingFooter}>
      ...
      <div style={styles.actionHint}>👆 Click to view All Meetings</div>
    </div>
  </div>
);
...
const styles = {
  ...
  kpiGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem', marginBottom: '2rem' },
  kpiCard: {
    padding: '2rem',
    borderRadius: '20px',
  ...
  kpiActive: { transform: 'translateY(-8px)', boxShadow: '0 20px 40px rgba(0,0,0,0.15)' },
  kpiIcon: { fontSize: '2.5rem', width: '60px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  kpiContent: { flex: 1 },
  kpiValue: { fontSize: '2.8rem', fontWeight: 800, color: '#1a1a1a', lineHeight: 1 },
  kpiLabel: { color: '#64748b', fontSize: '1rem', fontWeight: 600, textTransform: 'uppercase' },
  searchPanel: { background: 'white', borderRadius: '24px', padding: '2.5rem', boxShadow: '0 20px 40px rgba(0,0,0,0.1)', marginBottom: '2rem' },
  panelHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' },
  h2: { fontSize: '1.8rem', fontWeight: 700, color: '#1a1a1a', margin: 0 },
  searchGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' },
  searchInput: { width: '100%', padding: '1.2rem 1.5rem', border: '2px solid #e2e8f0', borderRadius: '16px', fontSize: '1rem', background: '#fafbfc' },
  clearBtn: { background: '#ef4444', color: 'white', border: 'none', padding: '1rem 2rem', borderRadius: '12px', fontWeight: 600, cursor: 'pointer' },
  meetingsSection: { background: 'white', borderRadius: '24px', boxShadow: '0 20px 40px rgba(0,0,0,0.1)', overflow: 'hidden' },
  sectionHeader: { padding: '2rem 2.5rem', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between' },
  meetingsGrid: { padding: '2.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(450px, 1fr))', gap: '2rem' },
  meetingCard: { background: 'linear-gradient(145deg, #ffffff, #f0f2f5)', borderRadius: '20px', padding: '2.5rem', border: '1px solid #e8ecf4', cursor: 'pointer', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' },
  meetingHeader: { display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' },
  meetingTitle: { fontSize: '1.5rem', fontWeight: 700, color: '#1a1a1a', margin: '0 0 0.5rem 0' },
  meetingDate: { color: '#64748b', fontSize: '1rem' },
  meetingType: { background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', padding: '0.5rem 1.5rem', borderRadius: '25px', fontSize: '0.85rem', fontWeight: 600 },
  meetingDesc: { color: '#475569', lineHeight: '1.7', marginBottom: '1.5rem', padding: '1.5rem', background: 'rgba(255,255,255,0.7)', borderRadius: '12px', borderLeft: '4px solid #667eea' },
  meetingFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1rem', borderTop: '1px solid #e2e8f0' },
  department: { background: '#f1f5f9', color: '#475569', padding: '0.5rem 1rem', borderRadius: '20px', fontSize: '0.9rem' },
  actionHint: { color: '#667eea', fontWeight: 600, fontSize: '0.95rem' },
  emptyState: { textAlign: 'center', padding: '6rem 2rem', color: '#94a3b8' },
  emptyIcon: { fontSize: '5rem', marginBottom: '2rem', opacity: 0.5 },
  createLink: { color: '#667eea', textDecoration: 'none', fontWeight: 600 },
  spinner: { width: '24px', height: '24px', border: '3px solid #e2e8f0', borderTop: '3px solid #667eea', borderRadius: '50%', animation: 'spin 1s linear infinite' },
};
