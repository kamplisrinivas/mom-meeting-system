...
export default function MomPointForm({ meetingId, token }) {
  ...
  if (!pointText.trim()) {
    alert("Please enter a MOM point");
    return;
  }
  if (!meetingId) {
    alert("Error: Meeting ID is missing");
    return;
  }

  // 🔹 Debugging log
  console.log("POST /api/mom body:", { meeting_id: meetingId, point: pointText });
  ...
  try {
    ...
    const data = await res.json();
    console.log("POST /api/mom response:", data);

    if (data.success) {
      setPointText("");
      fetchMomPoints();
    } else {
      alert(data.message || "Failed to add MOM point");
    }
  } catch (err) {
    console.error("Error creating MOM point:", err);
  }
};
        ...
        {actionItems.map((a) => (
          <div key={a.id} style={actionCard}>
            ...
            <div style={actionDetails}>
              ...
              {a.due_date ? new Date(a.due_date).toLocaleDateString() : "N/A"} | Status: {a.status}
            </div>
          </div>
        ))}
      </div>
...
const button = { padding: "8px 15px", background: "#003366", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" };
const toggleButton = { padding: "5px 10px", background: "#0066cc", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "13px" };
const momCard = { background: "#f9f9f9", padding: "15px", borderRadius: "6px", marginBottom: "12px", borderLeft: "4px solid #003366" };
const momText = { fontSize: "15px", marginBottom: "10px" };
const actionCard = { background: "#fff", padding: "6px 10px", borderRadius: "4px", border: "1px solid #ddd", marginBottom: "5px" };
